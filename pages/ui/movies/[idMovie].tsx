        ...
        <main id="main_movie_page">
            <div className="movie_page relative">
                <div className="bg_image_container">
                    <div className="background_image bg-cover xl:bg-contain h-[calc(100vh_-_var(--nav-height,0))]" style={{
                        backgroundImage: `url(${ConfigService.themoviedb.urls.image_view}/original${movie?.backdrop_path})`
                    }}>
                    </div>
                </div>
                <div className="movie_details lg:w-[100vw] h-[70vh] overflow-clip xl:justify-center mb-[5vh]">
                    {movie?.title && (
                        <h2 className={`main_title leading-none ${movieTitleSize(movie.title)} xl:w-[30vw]`}>
                            {movie.title}
                        </h2>
                    )}
                    <p className="font-bold xl:w-[30vw] mr-5" style={{textAlign: 'justify'}}>{movie?.overview}</p>
                    <p className="mt-5 flex flex-row flex-wrap items-center space-x-4 justify-center md:justify-start">
                        <span>{movie?.release_date.slice(0,4)}</span>  {movie?.genres.map((genre) => (<span className="genre">{genre.name}</span>))}
                    </p>
                    <p className="relative mt-5">
                        {movie?.vote_average.toFixed(1) >= 7.5 && <span className="absolute left-[0] top-[6px] w-6 h-6 "><i className="fa-solid fa-fire fa-xl fa-beat-fade text-secondary-500"></i></span>}
                        {movie?.vote_average.toFixed(1) < 5.0 && <span className="absolute left-[0] top-[6px] w-6 h-6 "><i className="fa-solid fa-face-frown fa-xl text-primary-400"></i></span>}
                        <span
                            className="font-extrabold text-3xl text-secondary-500 -tracking-widest absolute left-[35px]">{movie?.vote_average.toFixed(1)}
                        </span>
                        <span className="absolute text-sm left-[85px] top-[2.5px] tracking-wide"><span
                            className="font-extralight text-neutral-400">/</span>10</span>
                    </p>
                </div>
                <div className="about_movie absolute top-[50vh] md:top-[55vh] lg:top-[55vh] xl:top-[60vh] bottom-[0] left-[3vw] right-0 flex flex-col md:flex-row">
                    ...
                    <div className="similar_movies grow-0 max-w-[100vw] lg:max-w-[20vw] xl:max-w-[14vw] md:mr-10">
                        ...
                        :
                            movie?.similar.length > 0 &&<SimilarShowcase movie={movie} nbToShow={4} title={"Similar"} />}
                    </div>
                </div>
            </div>
        </main>
