...
/**
 * @swagger
 * /api/users/{idUser}/movies/seen:
 *  get:
 *      tags:
 *       - Utilisateur
 *      summary: Retourne la liste des films vus par un utilisateur by ID
 *      description: Retourne la liste des films vus par un utilisateur by ID
 *      parameters:
 *       - in: path
 *         name: idUser
 *         type: string
 ...
 *      responses:
 *          200:
 *              description: La liste des films vus par l'utilisateur
 */
    ...
    switch (req.method) {
        case "GET":
            ...
            if(resMongo && movies.length > 0){
                ...
                res.status(404).json({ status: 404, error: "L'utilisateur ne dispose d'aucun film dans ses déjà vu." });
            }
            break;
        default:
