...
import { ConfigService } from 'src/services/IMDB.API/config.service.js';
...
describe('API Handler for Discover', () => {
    ...
    it('should return data from the discover API when successful', async () => {
        const mockData = {
            results: [
                { id: 1, title: 'IdMovie 1' },
                { id: 2, title: 'IdMovie 2' }
            ]
        };
    ...
    it('should handle errors in configuration', async () => {
        jest.replaceProperty(ConfigService.themoviedb.keys, 'API_TOKEN', jest.fn().mockReturnValue('MY_API_TOKEN_'));
        const req = {};
        const res = {
            json: jest.fn(),
            status: jest.fn().mockReturnThis()
